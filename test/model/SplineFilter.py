...
class TestSplineFilter(torch.nn.Module):
    def __init__(
            self,
            filter_channels: int,
            filter_scale: int,
    ...
    def forward(
        ...
        eval_eigs_0 = BSpline(knots=self.filter_knots,
                              degree=self.filter_degree,
                              eval_eigs=eval_eigs)          # B x N x n_bases;

        F = self.filter_channels
        B = eval_eigs_0.shape[0]
        N = eval_eigs_0.shape[1]

        eval_eigs = eval_eigs_0.unsqueeze(0).expand(
            F, -1, -1, -1
        ...
        # === Serial-Vectorized Comparison ===
        eval_eigs = torch.unbind(eval_eigs_0, dim=0)
        filter_coeffs = torch.unbind(self.filter_coeffs, dim=-1)

        spectral_filter = []
        for i in range(B):
            filter_i = [eval_eigs[i] @ coeffs for coeffs in filter_coeffs]
            filter_i = torch.stack(filter_i, dim=0).T    # N x F
            spectral_filter.append(filter_i)
        spectral_filter = torch.stack(spectral_filter, dim=0)    # B x N x F
        spectral_filter = torch.square(spectral_filter)

        fourier_mod_serial = eval_x * spectral_filter

        assert torch.allclose(fourier_mod, fourier_mod_serial)
        # === //////////////////////////// ===
